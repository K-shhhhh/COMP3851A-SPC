...
Important Changes: 	Removed Sessions table. 
            		Defined and added ENUM for processing_status in Attachments table.
            		Removed "unique" constraint from group_name in Groups table.
            		Added processing_progress, processing_error, last_updated_at attribute to Attachments table.
					Replaced deleted_at attribute with is_deleted attribute in Channels and Attachments tables.
					Added source_page and source_type attributes to Chunks table.
					Added constraints to all remaining tables except User_Activity_Logs table.

Notes: 	Some UUIDs as foreign keys are set to have a default value of '00000000-0000-0000-0000-000000000000' to allow deletion in the tables they are referenced.
		Constraints for User_Activity_Logs are yet to be defined.
*/

create extension if not exists vector;
create type activity_status as enum ('active', 'deactivated', 'deleted');
...
-- System user for user account deletions
insert into users (user_id, email, fullname, password_hash, user_role, status, created_at)
values ('00000000-0000-0000-0000-000000000000', 'deleted-user@system.local', 'Deleted User', 'not-a-login-account', 'student', 'deleted', now() )
...
create table if not exists messages (
    ...
    user_id uuid default '00000000-0000-0000-0000-000000000000' not null, --default uuid for user account deletion
...
create table if not exists attachments (
    ...
    uploaded_by uuid default '00000000-0000-0000-0000-000000000000' not null, --default uuid for user account deletion
    channel_id uuid,
	group_id uuid,
    message_id bigint,
	title text not null,
...
create table if not exists knowledge_nodes (
	...
	references knowledge_graphs(graph_id)	on delete cascade
);
...
create table if not exists ai_response_sources (
	...
	references ai_responses(response_id)	on delete cascade,
	...
	references knowledge_nodes(node_id)	on delete set null,
